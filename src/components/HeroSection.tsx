  ...
  return (
    <section className="relative pt-28 pb-16 flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 text-center">
        <div className="inline-flex items-center px-4 py-1.5 mb-4 glass-card rounded-full">
          <Sparkles className="h-4 w-4 text-accent mr-2" />
          <span className="text-sm text-foreground">IA de nouvelle génération</span>
        </div>

        <h1 className="text-3xl md:text-5xl font-bold mb-3 text-foreground leading-tight">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl mx-auto">
          <Link to="/gallery" className="glass-card rounded-2xl p-6 text-left hover:scale-[1.02] transition-transform group">
            <Images className="h-8 w-8 text-accent mb-3" />
            <h3 className="text-xl font-semibold text-foreground mb-1">Galerie</h3>
            <p className="text-sm text-muted-foreground mb-4">Lecture gratuite des BD publiées par la communauté.</p>
            <Button variant="outline" size="sm" className="group-hover:bg-accent group-hover:text-accent-foreground">
              Parcourir la galerie
            </Button>
          </Link>
          <Link to="/premium-store" className="glass-card rounded-2xl p-6 text-left hover:scale-[1.02] transition-transform group border-accent/30">
            <ShoppingBag className="h-8 w-8 text-accent mb-3" />
            <h3 className="text-xl font-semibold text-foreground mb-1">Bibliothèque Premium</h3>
            <p className="text-sm text-muted-foreground mb-4">Œuvres exclusives à acheter et soutenir leurs auteurs.</p>
